...
def k_Anonymity(request):
	...
	for filename in os.listdir(path):
		...
		if os.path.isdir(filepath):
			s.append(filename)
	return render(request, 'k_Anonymity/k_Anonymity.html', locals())
...
def handle_upload_file(f):
	if(f.name.split(".")[1] == 'json'):
		...
		fs.save('upload/k_Anonymity/'+f.name.replace("_dict",'').split(".")[-2]+'/'+f.name, f)
	else:
		fs = FileSystemStorage()
		fs.save('upload/k_Anonymity/'+f.name.split(".")[-2]+'/'+f.name, f)
...
def File_List_Upload(request):
	...
	return render(request, 'k_Anonymity/file_list_upload.html', locals())

def File_List_Output(request):
	path = 'output/k_Anonymity/'
	s = []
	for dirPath, dirNames, fileNames in os.walk(path):
		for f in fileNames:
			file = os.path.join(dirPath, f)
			if not os.path.isdir(file):		#判斷是否是文件夾, 不是文件夾才開
				if(file.split(".")[1] == 'csv'):
					s.append(os.path.join(dirPath, f).replace('\\','/'))
	return render(request, 'k_Anonymity/file_list_output.html', locals())
