...
def l_Diversity(request):
	...
	for filename in os.listdir(path):
		...
		if os.path.isdir(filepath):
			s.append(filename)
	return render(request, 'l_Diversity/l_Diversity.html', locals())
...
def Web_View_CSV(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			upload_form.file = form.cleaned_data['file']
...
def handle_upload_file(f):
	...
	if(f.name.split(".")[1] == 'json'):
		...
		fs.save('upload/l_Diversity/'+f.name.replace("_dict",'').split(".")[-2]+'/'+f.name, f)
	else:
		fs = FileSystemStorage()
		fs.save('upload/l_Diversity/'+f.name.split(".")[-2]+'/'+f.name, f)
...
def File_List_Upload(request):
	...
	return render(request, 'l_Diversity/file_list_upload.html', locals())

def File_List_Output(request):
	path = 'output/l_Diversity/'
	s = []
	for dirPath, dirNames, fileNames in os.walk(path):
		for f in fileNames:
			file = os.path.join(dirPath, f)
			if not os.path.isdir(file):		#判斷是否是文件夾, 不是文件夾才開
				if(file.split(".")[1] == 'csv'):
					s.append(os.path.join(dirPath, f).replace('\\','/'))
	return render(request, 'l_Diversity/file_list_output.html', locals())
