{% extends "general/parallel_template.vue" %}
{% load i18n %}
...
{% block hint_area %}
    ...
    >
        <cell_introduction
            :introduction = "{
                content: '{% trans '1：請確認左邊的參數設定，進行下一步的調整' %}',
                is_chinese: '{{ LANGUAGE_CODE }}' === 'zh-hant',
            }"
        ></cell_introduction>
        <cell_introduction
            :introduction = "{
                content: '{% trans '2：請確認右邊的檔案預覽' %}',
                is_chinese: '{{ LANGUAGE_CODE }}' === 'zh-hant',
            }"
        ></cell_introduction>        
        <cell_introduction
            :introduction = "{
                content: '{% trans '3：設定完成後，將頁面滾動至最下方，並按下繼續執行' %}',
                is_chinese: '{{ LANGUAGE_CODE }}' === 'zh-hant',
            }"
...
{% block right_container %}
    <div id="preview_file" class="form-group col-md-6">
        ...
        {% include 'general/display_file.vue' with title=preview_file display_url=upload_display_url id="preview" %}
    </div>
...
{% block script_area %}
    <script>
        ...
        title_area.title.content = "{% trans '資料關係可藉由自行輸入或下拉式選單填入' %}";
        title_area.title.is_chinese = '{{ LANGUAGE_CODE }}' === 'zh-hant';
        title_area.introduction.content = "{% trans '閱讀完注意事項後，請於下方區塊進行後續設定，設定完成後按下繼續執行' %}";
        title_area.introduction.is_chinese = '{{ LANGUAGE_CODE }}' === 'zh-hant';
    </script>
    
    <script>
        function reset() {
            {% if custom_mode == 'DPView' or advanced_settings %}
                var number_title_list = {{ number_title_list|safe }};
                
                for (var number_title of number_title_list) {
                    document.getElementById(number_title+'_relationship_select_pair').value = '';
                }
            {% endif %}
            ...
            {% if custom_mode == 'json_parser' %}
                ...
                for (var column_title in string_element_dict) {
                    document.getElementById(column_title+'_relationship_select_pair').value = '';
                }
            {% endif %}
        }
    ...
    {% if custom_mode == 'DPView' %}
        <script>
            function execute() {
                ...
                var url = "{% url 'create_DPView_json' %}";
                var data = {
                    'csv_name' : '{{ file_name }}',
    ...
    {% elif custom_mode == 'json_parser' %}
        <script>
            function execute() {
                ...
                if (lose_required) {
                    danger_alert("{% trans '尚有對應關係還沒填寫，未填寫的欄位已由紅色標出' %}");
                }
                else {
                    var url = "{% url 'create_json' %}";
                    data = {
                        'csv_name' : '{{ file_name }}',
