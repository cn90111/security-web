...
class MachineLearning():
    ...
    def __init__(self, method, train_file_path, test_file_path):
    ...
    def one_hot_encoding(self, dataframe1, dataframe2):
        encoding_dict = self.get_encoding_dict(dataframe1, dataframe2)
        list1 = self.encoding(dataframe1, encoding_dict)
        list2 = self.encoding(dataframe2, encoding_dict)
        return list1, list2
            
    def need_one_hot(self, data1, data2):
        if type(data1) is str or type(data1) is chr:
            return True
        if type(data2) is str or type(data2) is chr:
            return True
        return False
    ...
    def get_encoding_dict(self, dataframe1, dataframe2):
        ...
        for column_title in dataframe1:
            ...
            data2 = None
            i = 0
            while (not data1 or not data2) and (i < dataframe1.shape[0] and i < dataframe2.shape[0]):
                if not data1:
                    data1 = dataframe1.loc[i, column_title]
                if not data2:
                    data2 = dataframe2.loc[i, column_title]
                i = i + 1
            if self.need_one_hot(data1, data2):
                elements1 = self.get_elements(data1, dataframe1.loc[:, column_title])
                elements2 = self.get_elements(data2, dataframe2.loc[:, column_title])
                encoding_dict[column_title] = list(elements1.union(elements2))
        return encoding_dict
        
    def encoding(self, dataframe, encoding_dict):
        encoding_list = []
